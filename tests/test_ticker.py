...
def test_filter_of_tickers_from_csv():
    csv_string = """<ticker>,<date>,<open>,<high>,<low>,<close>,<vol>
    A,20190722,68.91,69.465,68.46,69.09,3103300
    AA,20190722,23.18,23.3901,22.695,22.98,3719200
    AAC,20190722,0.97,1.02,0.88,0.9099,127600
    AAN,20190722,63.42,64.52,62.98,63.03,563500
    AAP,20190722,157.28,160.17,157.24,158.9,840700
    ABP,20190722,157.28,160.17,157.24,,840700
    """
    f = io.StringIO(csv_string)
    # only ticker == A is between 68 and 70 in the close field
    tickers = fetch_unique_tickers(
        f, lambda v: True if 68 < Decimal(v["<close>"]) < 70 else False
    )
    assert len(tickers) == 1


def test_filter_of_tickers_from_csv_with_duplicates():
